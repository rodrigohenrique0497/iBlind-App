...
const STEPS = [
  { id: 'CLIENT', label: 'ENTRADA', icon: <User size={18} /> },
...
export const NewServiceWizard: React.FC<WizardProps> = ({ inventory, onComplete, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col h-full overflow-hidden z-[500] animate-premium-in">
      {/* Header com indicador de passos mais afastado do conteúdo */}
      <header className="h-24 border-b border-white/5 bg-black/90 backdrop-blur-2xl flex items-center justify-between px-8 shrink-0 relative z-10">
        <button 
          onClick={back} 
          className="w-12 h-12 flex items-center justify-center bg-white/5 rounded-2xl text-white/40 hover:text-white transition-all active:scale-90 border border-white/5"
        >
          <ArrowLeft size={20}/>
        </button>
        ...
        <div className="flex gap-3 px-4 py-2 bg-white/5 rounded-full border border-white/5">
          {STEPS.map((s, i) => (
            <div 
              key={s.id} 
              className={`h-1.5 rounded-full transition-all duration-700 ${i <= step ? 'w-10 bg-white' : 'w-2 bg-white/10'}`} 
            />
          ))}
        </div>
        ...
        <button 
          onClick={onCancel}
          className="w-12 h-12 flex items-center justify-center bg-red-500/5 rounded-2xl text-red-500/40 hover:text-red-500 transition-all border border-red-500/10"
        >
          <X size={20}/>
        </button>
      </header>
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar scroll-smooth">
        {/* Padding Superior aumentado para evitar proximidade excessiva com o header */}
        <div className="max-w-xl mx-auto px-6 pt-20 pb-40 space-y-16">
          ...
          <div className="space-y-4 text-center md:text-left">
            ...
            <h1 className="text-4xl brand-font-bold tracking-tighter text-white uppercase leading-none">
              Novo Atendimento
            </h1>
          ...
          {step === 0 && (
            <div className="space-y-10 animate-premium-in">
              <div className="grid gap-8">
                ...
                />
                <IBInput 
                  label="APARELHO E MODELO" 
                  placeholder="Ex: iPhone 15 Pro Max" 
                  value={data.deviceModel} 
                  error={errors.deviceModel}
                  onChange={e => setData({...data, deviceModel: e.target.value})} 
                />
              </div>
              ...
              <div className="pt-6">
                <IBCard className="border-dashed bg-white/[0.01] hover:bg-white/[0.03] transition-colors p-8">
                  <IBImageUpload 
                    label="EVIDÊNCIAS DO ESTADO INICIAL" 
                    images={data.photos || []} 
          ...
          {step === 1 && (
            <div className="space-y-12 animate-premium-in">
              <div className="grid gap-10">
                ...
                />
                <IBBinaryCheck 
                  label="ESTADO DA TRASEIRA (VIDRO)" 
              ...
              <div className="pt-8 border-t border-white/5">
                <IBInput 
                  ...
                  onChange={e => setData({...data, deviceIMEI: e.target.value})} 
                />
              </div>
            </div>
          )}
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {(['FULL', 'SCREEN', 'BACK'] as ServiceCoverage[]).map((cov) => (
                    <button 
                      key={cov}
                      onClick={() => setData({...data, coverage: cov})}
                      className={`w-full p-8 rounded-3xl flex items-center justify-between border transition-all duration-500 group ${
                        data.coverage === cov 
                        ? 'bg-white text-black border-white shadow-[0_30px_60px_-15px_rgba(255,255,255,0.1)] scale-[1.03] z-20' 
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div className="text-left">
                          <span className="font-black text-base uppercase tracking-tight block">
                            {cov === 'FULL' ? 'BLINDAGEM 360' : cov === 'SCREEN' ? 'BLINDAGEM FRONTAL' : 'BLINDAGEM TRASEIRA'}
                          </span>
                          <span className={`text-[9px] font-bold uppercase tracking-widest mt-1 block ${data.coverage === cov ? 'text-black/40' : 'text-white/20'}`}>
                            {cov === 'FULL' ? 'Proteção total do dispositivo' : 'Foco no display principal'}
                          </span>
                        </div>
                      </div>
                      {data.coverage === cov && <CheckCircle size={24} className="animate-in fade-in zoom-in duration-500" />}
                    </button>
                  ))}
                </div>
              </div>
                ...
                <div className="relative group">
                   ...
                   <div className="flex items-baseline gap-4">
                     ...
                     <input 
                       type="number"
                       placeholder="0.00"
                       className={`w-full bg-transparent text-6xl font-black outline-none border-b-2 py-4 transition-all tracking-tighter ${errors.valueBlindagem ? 'border-red-500' : 'border-white/10 focus:border-white'}`}
                ...
                <div className="grid grid-cols-2 gap-4">
                  {(['PIX', 'CREDITO', 'DEBITO', 'DINHEIRO'] as PaymentMethod[]).map(method => (
                    <button 
                      ...
                      className={`py-6 rounded-2xl border text-[11px] font-black tracking-widest uppercase transition-all duration-300 ${
          ...
          {step === 3 && (
            <div className="space-y-12 animate-premium-in">
              <div className="space-y-6">
                ...
                <p className="text-xs font-medium text-white/40 leading-relaxed uppercase tracking-wider bg-white/5 p-6 rounded-2xl border border-white/5">
                  Ao assinar, o cliente confirma a vistoria realizada e aceita os termos de garantia vitalícia da blindagem iBlind.
                </p>
              ...
              <div className="p-10 bg-white/5 border border-white/5 rounded-[40px] flex justify-between items-center shadow-inner">
      ...
      <footer className="p-8 bg-black/90 backdrop-blur-3xl border-t border-white/5 sticky bottom-0 z-10">
        <div className="max-w-xl mx-auto flex gap-4">
          <IBButton 
            onClick={next} 
            className="flex-1 h-20 rounded-[32px] text-sm"
